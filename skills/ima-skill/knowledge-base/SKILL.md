...
```bash
# 有扩展名（自动推断）
node skills/ima-skill/knowledge-base/scripts/preflight-check.cjs --file report.pdf

# 无扩展名或扩展名不可识别（需传入 content-type，如从 HTTP HEAD 获取）
node skills/ima-skill/knowledge-base/scripts/preflight-check.cjs --file downloaded_file --content-type application/pdf
```
...
PREFLIGHT=$(node skills/ima-skill/knowledge-base/scripts/preflight-check.cjs \
...
# 5. 上传文件到 COS
node skills/ima-skill/knowledge-base/scripts/cos-upload.cjs \
  --file "/path/to/report.pdf" \
